...
def load_in_csv_file():
    ...
    if uploaded_file is not None:
        ...
        raw_export_transaction = pl.scan_csv(
            stringio, separator=";", schema=column_type_mapping, has_header=True
        ).collect()
        parsed_for_type_df = raw_export_transaction.with_columns(
            pl.col("id").str.to_integer(),
            pl.col("price").str.to_integer(),
            pl.col("created_timestamp").str.to_datetime(
                format="%Y-%m-%d %H:%M:%S%.f", strict=True
            )
            + pl.duration(hours=2),
        )
