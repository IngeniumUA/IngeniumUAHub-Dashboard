...
from app.modules.core.core_parsing import parse_transactions_to_df, parse_checkouttrackers_to_df, parse_checkouts_to_df
...
class CoreSyncManager:
    ...
    @abstractmethod
    def fetch_data(self, core_client: CoreClient, limit: int, offset: int, **kwargs) -> list[dict]:
        return []
    ...
    def sync_once(self, from_scratch: bool = False) -> bool:
        ...
        for _ in range(self.allowed_total_requests):
            fetched_data += self.fetch_data(core_client=core_client, limit=min(count_in_core, self.max_objects_per_request), offset=len(fetched_data))
            if len(fetched_data) >= count_in_core:
                break
        else:
            # fixme: this will also trigger if it needed exactly this amount of requests
            st.toast(f"Could not sync {self.table_name} in {self.allowed_total_requests} requests")
        ...
        # Load into DB
        if from_scratch:
            stmt = """SELECT COUNT(*) FROM duckdb_tables WHERE table_name = $table_name"""
            result = duck.execute(stmt, {"table_name": self.table_name}).fetchone()
            if result[0] > 0:  # If table exists (count > 0), drop it
                duck.execute(f"DROP TABLE $table_name", {"table_name": self.table_name})

        # Register the Polars DataFrame, then create
        duck.register(f"temp_{self.table_name}_df", source_df)
        duck.execute(f"CREATE TABLE {self.table_name} AS SELECT * FROM temp_{self.table_name}_df")
