...
class UmamiClient:
    def __init__(self, username: str, password: str, base_url: str, connection_timeout: float = 10.0):
    ...
    def _request_token(self) -> Optional[UmamiToken]:
        with Client(base_url=self.base_url, timeout=self.connection_timeout) as client:
            response = client.post("/api/auth/login", json={"username": self.username, "password": self.password})
